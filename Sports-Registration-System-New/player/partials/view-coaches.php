<?php
$stmt = $db_con->prepare("SELECT c.*, s.name as sport_name, t.name as team_name FROM `coaches` c LEFT JOIN `sports` s ON c.sport_id = s.id LEFT JOIN `teams` t ON c.team_id = t.id ORDER BY c.`full_name` ASC");
$stmt->execute();
